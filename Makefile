...
clean:
	./mvnw clean
	rm -f dists/pepe-munin-${RPM_VER}*.rpm
...
dist: api
    ...
    cp -v pepe-munin-${VERSION}-SNAPSHOT.jar lib/pepe-munin.jar && \
    fpm -s dir \
        ...
        --vendor 'Globo.com' \
        --description "Pepe Munin service" \
        --after-install scripts/postinstall \
        -f -p ../dists/pepe-munin-${RPM_VER}.el7.noarch.rpm lib/=/opt/pepe/lib/ scripts/=/opt/pepe/scripts/ conf/=/opt/pepe/conf/ initscript=/etc/init.d/pepe && \
    cd $$old
